...
def test_base64_decodes_woff2_data_uri():
    handler = Base64FileHandler()
    result = handler.execute("data:font/woff2;base64,AAAA")
    assert result[0].filename.endswith(".woff2")


def test_base64_decodes_gif_data_uri():
    handler = Base64FileHandler()
    result = handler.execute("data:image/gif;base64,AAAA")
    assert result[0].filename.endswith(".gif")


def test_base64_jpeg_data_uri_yields_jpg():
    handler = Base64FileHandler()
    result = handler.execute("data:image/jpeg;base64,AAAA")
