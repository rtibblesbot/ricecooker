...
# if any changes are needed in the files served out of the "samples" folder,
# you need to delete the .webcache folder that is generated at the project root
# for the tests to be rerun
...
def http_local_server():
    ...
    def spawn_http_server(arg):
        with http.server.HTTPServer(("", PORT), Handler) as httpd:
            ...
            # the test test_pretextbook_css_fetch below, see the docs on that
            # test method for more info
...
class TestArchiver(unittest.TestCase):
    ...
    def test_archive_path_as_relative_url(self):
        link = "../kolibri.png?1.2.3"
        page_link = "https://learningequality.org/team/index.html"
        page_filename = downloader.get_archive_filename(
            page_link, download_root="./"
        )
    ...
    # If any changes are needed in the files served out of the "samples"
    # folder, you need to delete the .webcache folder that is generated at the
    # project root for the tests to be rerun
    #
    # This test relies on behavior in the embedded http server declared above
    # as a class level fixture to treat any extensionless file as having a mime
    # type of text/css.
    # Handler.extensions_map = {'': 'text/css'}
    # if another test needs different behavior this may need to be customized
    # or have the fixture be scoped to this test but for now it seems useful to
    # share the resource between this and other future tests.
    def test_pretextbook_css_fetch(self):
        sushi_url = (
            "http://localhost:" + str(PORT)
            + "/samples/PreTeXt_book_test/activecalculus.org"
            "/single2e/sec-5-2-FTC2.html"
        )
        dest_dir = (
            "active_calc_2e_again_" + datetime.now().strftime("%Y-%m-%d_%H:%M:%S")
        )
        ...
        try:
            ...
            book_dest_dir = (
                downloads_dir / dest_dir / "localhost:8181" / "samples"
                / "PreTeXt_book_test"
            )
            with open(
                book_dest_dir / "activecalculus.org" / "single2e"
                / "sec-5-2-FTC2.html",
                "r",
            ) as file:
                page_html = file.read()
                assert (
                    ...
                    "css2_family_Material+Symbols+Outlined:"
                    "opsz,wght,FILL,GRAD@24,400,0,0/index.css"
                    in page_html
                )

            css_path = (
                book_dest_dir / "fonts.googleapis.com"
                / "css2_family_Material+Symbols+Outlined:"
            ...
            with open(css_path, "r") as file:
                ...
                assert (
                    'src: url("../../fonts.gstatic.com/s/'
                    "materialsymbolsoutlined" in css_file_contents
                )

            font_size = os.path.getsize(
                book_dest_dir / "fonts.gstatic.com" / "s"
                / "materialsymbolsoutlined" / "v290" / "material_symbols.woff"
            )
            assert font_size > 0
        finally:
